...
public final class DoubleMath {
    ...
    static double roundIntermediate(double d, RoundingMode roundingMode) {
        ...
        switch (AnonymousClass1.$SwitchMap$java$math$RoundingMode[roundingMode.ordinal()]) {
            case 1:
                ...
                return d;
            case 2:
                return (d >= AudioStats.AUDIO_AMPLITUDE_NONE || isMathematicalInteger(d)) ? d : ((long) d) - 1;
            case 3:
                return (d <= AudioStats.AUDIO_AMPLITUDE_NONE || isMathematicalInteger(d)) ? d : ((long) d) + 1;
            case 4:
                return d;
            case 5:
                ...
                return ((long) d) + (d > AudioStats.AUDIO_AMPLITUDE_NONE ? 1 : -1);
            case 6:
                return Math.rint(d);
            case 7:
    ...
    public static BigInteger roundToBigInteger(double d, RoundingMode roundingMode) {
        ...
        return dRoundIntermediate < AudioStats.AUDIO_AMPLITUDE_NONE ? bigIntegerShiftLeft.negate() : bigIntegerShiftLeft;
    }

    public static boolean isPowerOfTwo(double d) {
        if (d <= AudioStats.AUDIO_AMPLITUDE_NONE || !DoubleUtils.isFinite(d)) {
            return false;
        }
    ...
    */
    ...
    public static boolean isMathematicalInteger(double d) {
        return DoubleUtils.isFinite(d) && (d == AudioStats.AUDIO_AMPLITUDE_NONE || 52 - Long.numberOfTrailingZeros(DoubleUtils.getSignificand(d)) <= Math.getExponent(d));
    }
