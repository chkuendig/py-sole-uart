...
public final class Maps {
    ...
    private static <K, V> void doDifference(Map<? extends K, ? extends V> map, Map<? extends K, ? extends V> map2, Equivalence<? super V> equivalence, Map<K, V> map3, Map<K, V> map4, Map<K, V> map5, Map<K, MapDifference.ValueDifference<V>> map6) {
        for (Map.Entry<? extends K, ? extends V> entry : map.entrySet()) {
            ...
            if (map2.containsKey(key)) {
                Manifest.permission_group permission_groupVar = (Object) NullnessCasts.uncheckedCastNullableTToT(map4.remove(key));
                if (equivalence.equivalent(value, permission_groupVar)) {
                    map5.put(key, value);
                } else {
                    map6.put(key, ValueDifferenceImpl.create(value, permission_groupVar));
                }
            } else {
                map3.put(key, value);
            }
    ...
    static String toStringImpl(Map<?, ?> map) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            sbAppend.append(entry.getKey()).append(SignatureVisitor.INSTANCEOF).append(entry.getValue());
            z = false;
        }
