...
public class DefaultSsChunkSource implements SsChunkSource {
    ...
    public static final class Factory implements SsChunkSource.Factory {
        ...
        public SsChunkSource createChunkSource(LoaderErrorThrower loaderErrorThrower, SsManifest ssManifest, int i, ExoTrackSelection exoTrackSelection, TransferListener transferListener, CmcdConfiguration cmcdConfiguration) {
            ...
            return new DefaultSsChunkSource(loaderErrorThrower, ssManifest, i, exoTrackSelection, dataSourceCreateDataSource, cmcdConfiguration);
        }
    }
    ...
    private static MediaChunk newMediaChunk(Format format, DataSource dataSource, Uri uri, int i, long j, long j2, long j3, int i2, Object obj, ChunkExtractor chunkExtractor, CmcdHeadersFactory cmcdHeadersFactory) {
        ImmutableMap<String, String> immutableMapCreateHttpRequestHeaders;
        if (cmcdHeadersFactory == null) {
            immutableMapCreateHttpRequestHeaders = ImmutableMap.of();
        } else {
            immutableMapCreateHttpRequestHeaders = cmcdHeadersFactory.createHttpRequestHeaders();
        }
        return new ContainerMediaChunk(dataSource, new DataSpec.Builder().setUri(uri).setHttpRequestHeaders(immutableMapCreateHttpRequestHeaders).build(), format, i2, obj, j, j2, j3, -9223372036854775807L, i, 1, j, chunkExtractor);
    }
