...
class HlsChunkSource {
    ...
    public void getNextChunk(long j, long j2, List<HlsMediaChunk> list, boolean z, HlsChunkHolder hlsChunkHolder) {
        ...
        int i;
        String objectType;
        CmcdHeadersFactory objectType2;
        ...
        if (this.cmcdConfiguration == null) {
            ...
            CmcdHeadersFactory cmcdHeadersFactory = new CmcdHeadersFactory(this.cmcdConfiguration, this.trackSelection, j3, "h", !hlsMediaPlaylist.hasEndTag);
            if (getIsMuxedAudioAndVideo()) {
                objectType = "av";
            } else {
                objectType = CmcdHeadersFactory.getObjectType(this.trackSelection);
            }
            objectType2 = cmcdHeadersFactory.setObjectType(objectType);
        }
        Uri fullEncryptionKeyUri = getFullEncryptionKeyUri(hlsMediaPlaylist, nextSegmentHolder.segmentBase.initializationSegment);
        hlsChunkHolder.chunk = maybeCreateEncryptionChunkFor(fullEncryptionKeyUri, i, true, objectType2);
        ...
        hlsChunkHolder.chunk = maybeCreateEncryptionChunkFor(fullEncryptionKeyUri2, i, false, objectType2);
        if (hlsChunkHolder.chunk != null) {
            return;
        }
