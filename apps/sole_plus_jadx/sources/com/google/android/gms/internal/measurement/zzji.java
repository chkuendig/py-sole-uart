...
public abstract class zzji<T extends zzix> {
    ...
    static <T extends zzix> T zza(Class<T> cls) {
        ...
        if (cls.equals(zzix.class)) {
            ...
            if (!cls.getPackage().equals(zzji.class.getPackage())) {
        ...
        try {
            try {
                try {
                    return cls.cast(((zzji) Class.forName(str, true, classLoader).getConstructor(new Class[0]).newInstance(new Object[0])).zza());
            ...
            while (it.hasNext()) {
                try {
                    ...
                    Logger.getLogger(zzit.class.getName()).logp(Level.SEVERE, "com.google.protobuf.GeneratedExtensionRegistryLoader", "load", "Unable to load " + cls.getSimpleName(), (Throwable) e5);
                }
            }
            if (arrayList.size() == 1) {
