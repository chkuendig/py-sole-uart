...
public final class DynamicColor {
    ...
    public double getTone(DynamicScheme dynamicScheme) {
        ...
        if (function != null) {
            ...
            if ((dDoubleValue2 - dClampDouble) * d < delta) {
                double d2 = delta * d;
                dDoubleValue2 = MathUtils.clampDouble(AudioStats.AUDIO_AMPLITUDE_NONE, 100.0d, dClampDouble + d2);
                if ((dDoubleValue2 - dClampDouble) * d < delta) {
                    dClampDouble = MathUtils.clampDouble(AudioStats.AUDIO_AMPLITUDE_NONE, 100.0d, dDoubleValue2 - d2);
                }
            }
            if (50.0d > dClampDouble || dClampDouble >= 60.0d) {
                if (50.0d > dDoubleValue2 || dDoubleValue2 >= 60.0d) {
                    ...
                    dMin = d > AudioStats.AUDIO_AMPLITUDE_NONE ? 60.0d : 49.0d;
                } else if (d > AudioStats.AUDIO_AMPLITUDE_NONE) {
                    dMin = Math.max(dDoubleValue2, (delta * d) + 60.0d);
                    dClampDouble = 60.0d;
                } else {
        ...
        if (Contrast.ratioOfTones(dMax, d3) >= contrast3 && Contrast.ratioOfTones(dMin2, d3) >= contrast3) {
            return d3;
        }
