...
public class HctSolver {
    ...
    static boolean isBounded(double d) {
        return AudioStats.AUDIO_AMPLITUDE_NONE <= d && d <= 100.0d;
    }
    ...
    static double[][] bisectToSegment(double d, double d2) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (dArrNthVertex[0] >= AudioStats.AUDIO_AMPLITUDE_NONE) {
    ...
    static double inverseChromaticAdaptation(double d) {
        ...
        return MathUtils.signum(d) * Math.pow(Math.max(AudioStats.AUDIO_AMPLITUDE_NONE, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d);
    }
    ...
    static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i < 5) {
            ...
            double d5 = dPow;
            double dPow2 = Math.pow(((d2 == AudioStats.AUDIO_AMPLITUDE_NONE || dSqrt == AudioStats.AUDIO_AMPLITUDE_NONE) ? 0.0d : d2 / Math.sqrt(d4)) * dPow, 1.1111111111111112d);
            double d6 = dSqrt;
            ...
            double d7 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * dCos) + ((dPow2 * 11.0d) * dCos2)) + ((dPow2 * 108.0d) * dSin));
            double d8 = d7 * dCos2;
            double d9 = d7 * dSin;
