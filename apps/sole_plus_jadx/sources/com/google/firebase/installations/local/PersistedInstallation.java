...
public class PersistedInstallation {
    ...
    private File getDataFile() {
        if (this.dataFile == null) {
            synchronized (this) {
                if (this.dataFile == null) {
                    this.dataFile = new File(this.firebaseApp.getApplicationContext().getFilesDir(), "PersistedInstallation." + this.firebaseApp.getPersistenceKey() + SdkConstants.DOT_JSON);
                }
            }
        }
    ...
    public PersistedInstallationEntry insertOrUpdatePersistedInstallationEntry(PersistedInstallationEntry persistedInstallationEntry) throws JSONException, IOException {
        ...
        try {
            ...
            fileCreateTempFile = File.createTempFile(SETTINGS_FILE_NAME_PREFIX, FileListingService.DIRECTORY_TEMP, this.firebaseApp.getApplicationContext().getFilesDir());
