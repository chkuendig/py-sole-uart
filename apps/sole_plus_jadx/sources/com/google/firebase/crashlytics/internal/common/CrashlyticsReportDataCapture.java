...
public class CrashlyticsReportDataCapture {
    ...
    static {
        ...
        map.put(SdkConstants.ABI_ARMEABI, 5);
        map.put(SdkConstants.ABI_ARMEABI_V7A, 6);
        map.put(SdkConstants.ABI_ARM64_V8A, 9);
        map.put("x86", 0);
        map.put("x86_64", 1);
    ...
    private CrashlyticsReport.Session.Device populateSessionDeviceData() {
        ...
        return CrashlyticsReport.Session.Device.builder().setArch(deviceArchitecture).setModel(Build.MODEL).setCores(iAvailableProcessors).setRam(jCalculateTotalRamInBytes).setDiskSpace(blockCount).setSimulator(zIsEmulator).setState(deviceState).setManufacturer(Build.MANUFACTURER).setModelClass(Build.PRODUCT).build();
    }
