    ...
    private static Map<String, String> splitQuery(String str) {
        ...
        while (true) {
            ...
            if (str.charAt(i) == '&') {
                ...
                if (iIndexOf < 0) {
                    if (iIndexOf2 < 0) {
                        strDecode = Uri.decode(str.substring(i).replace(SignatureVisitor.EXTENDS, ' '));
                    } else {
                        String strDecode3 = Uri.decode(str.substring(i, iIndexOf2).replace(SignatureVisitor.EXTENDS, ' '));
                        strDecode2 = Uri.decode(str.substring(iIndexOf2 + 1).replace(SignatureVisitor.EXTENDS, ' '));
                    ...
                    if (iIndexOf2 < 0 || iIndexOf2 > iIndexOf) {
                        String strDecode4 = Uri.decode(str.substring(i, iIndexOf).replace(SignatureVisitor.EXTENDS, ' '));
                        if (!map.containsKey(strDecode4)) {
                            map.put(strDecode4, "");
                        }
                    } else {
                        String strDecode5 = Uri.decode(str.substring(i, iIndexOf2).replace(SignatureVisitor.EXTENDS, ' '));
                        String strDecode6 = Uri.decode(str.substring(iIndexOf2 + 1, iIndexOf).replace(SignatureVisitor.EXTENDS, ' '));
            ...
            if (decodeHintType != DecodeHintType.CHARACTER_SET && decodeHintType != DecodeHintType.NEED_RESULT_POINT_CALLBACK && decodeHintType != DecodeHintType.POSSIBLE_FORMATS && (strSubstring = mapSplitQuery.get(decodeHintType.name())) != null) {
                if (decodeHintType.getValueType().equals(Object.class)) {
                    ...
                    if (strSubstring.isEmpty()) {
                        enumMap.put((EnumMap) decodeHintType, (DecodeHintType) Boolean.TRUE);
                    } else if ("0".equals(strSubstring) || SdkConstants.VALUE_FALSE.equalsIgnoreCase(strSubstring) || SdkConstants.VALUE_NO.equalsIgnoreCase(strSubstring)) {
