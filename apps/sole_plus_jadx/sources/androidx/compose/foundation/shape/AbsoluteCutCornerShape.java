...
public final class AbsoluteCutCornerShape extends CornerBasedShape {
    ...
    public Outline mo1251createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection) {
        if (topStart + topEnd + bottomStart + bottomEnd == 0.0f) {
            return new Outline.Rectangle(SizeKt.m4384toRectuvyYCjk(size));
        }
        Path Path = AndroidPath_androidKt.Path();
        Path.moveTo(0.0f, topStart);
        Path.lineTo(topStart, 0.0f);
        Path.lineTo(Size.m4363getWidthimpl(size) - topEnd, 0.0f);
        Path.lineTo(Size.m4363getWidthimpl(size), topEnd);
        Path.lineTo(Size.m4363getWidthimpl(size), Size.m4360getHeightimpl(size) - bottomEnd);
        Path.lineTo(Size.m4363getWidthimpl(size) - bottomEnd, Size.m4360getHeightimpl(size));
        Path.lineTo(bottomStart, Size.m4360getHeightimpl(size));
        Path.lineTo(0.0f, Size.m4360getHeightimpl(size) - bottomStart);
        Path.close();
