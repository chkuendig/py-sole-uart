...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m1392canReuse7_7YC6M(TextLayoutResult textLayoutResult, AnnotatedString annotatedString, TextStyle textStyle, List<AnnotatedString.Range<Placeholder>> list, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver resolver, long j) {
        TextLayoutInput layoutInput = textLayoutResult.getLayoutInput();
        if (textLayoutResult.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), annotatedString) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(textStyle) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), list) || layoutInput.getMaxLines() != i || layoutInput.getSoftWrap() != z || !TextOverflow.m7181equalsimpl0(layoutInput.getOverflow(), i2) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), resolver) || Constraints.m7210getMinWidthimpl(j) != Constraints.m7210getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z || TextOverflow.m7181equalsimpl0(i2, TextOverflow.INSTANCE.m7191getEllipsisgIe3tQ8())) {
            return Constraints.m7208getMaxWidthimpl(j) == Constraints.m7208getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m7207getMaxHeightimpl(j) == Constraints.m7207getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
