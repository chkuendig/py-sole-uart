...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m6713getLengthimpl(textFieldBuffer.getChanges().mo1410getRangejx7JFs(0)) != 1 || TextRange.m6713getLengthimpl(textFieldBuffer.getChanges().mo1409getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM6715getMinimpl = TextRange.m6715getMinimpl(textFieldBuffer.getChanges().mo1410getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != iM6715getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM6715getMinimpl);
        }
    }
