...
public final class SelectionManagerKt {
    ...
    public static final Rect getSelectedRegionRect(List<? extends Pair<? extends Selectable, Selection>> list, LayoutCoordinates layoutCoordinates) {
        ...
        while (i2 < size) {
            ...
            if (offset == offset2 || (layoutCoordinates2 = selectableComponent1.getLayoutCoordinates()) == null) {
                ...
                long jMo5966localPositionOfR5De75A = layoutCoordinates.mo5966localPositionOfR5De75A(layoutCoordinates2, jOffset);
                long jMo5966localPositionOfR5De75A2 = layoutCoordinates.mo5966localPositionOfR5De75A(layoutCoordinates2, jOffset2);
                left = Math.min(left, Offset.m4294getXimpl(jMo5966localPositionOfR5De75A));
                top = Math.min(top, Offset.m4295getYimpl(jMo5966localPositionOfR5De75A));
                right = Math.max(right, Offset.m4294getXimpl(jMo5966localPositionOfR5De75A2));
                bottom = Math.max(bottom, Offset.m4295getYimpl(jMo5966localPositionOfR5De75A2));
            }
            i2++;
            size = i;
            c = 0;
    ...
    private static final long m1685getMagnifierCenterJVtK1S4(SelectionManager selectionManager, long j, Selection.AnchorInfo anchorInfo) {
        ...
        if (anchorSelectable$foundation_release != null && (containerLayoutCoordinates = selectionManager.getContainerLayoutCoordinates()) != null && (layoutCoordinates = anchorSelectable$foundation_release.getLayoutCoordinates()) != null && (offset = anchorInfo.getOffset()) <= anchorSelectable$foundation_release.getLastVisibleOffset()) {
            Offset offsetM1672getCurrentDragPosition_m7T9E = selectionManager.m1672getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(offsetM1672getCurrentDragPosition_m7T9E);
            float fM4294getXimpl = Offset.m4294getXimpl(layoutCoordinates.mo5966localPositionOfR5De75A(containerLayoutCoordinates, offsetM1672getCurrentDragPosition_m7T9E.m4304unboximpl()));
            long jMo1633getRangeOfLineContainingjx7JFs = anchorSelectable$foundation_release.mo1633getRangeOfLineContainingjx7JFs(offset);
            if (TextRange.m6711getCollapsedimpl(jMo1633getRangeOfLineContainingjx7JFs)) {
                fCoerceIn = anchorSelectable$foundation_release.getLineLeft(offset);
            } else {
                float lineLeft = anchorSelectable$foundation_release.getLineLeft(TextRange.m6717getStartimpl(jMo1633getRangeOfLineContainingjx7JFs));
                float lineRight = anchorSelectable$foundation_release.getLineRight(TextRange.m6712getEndimpl(jMo1633getRangeOfLineContainingjx7JFs) - 1);
                fCoerceIn = RangesKt.coerceIn(fM4294getXimpl, Math.min(lineLeft, lineRight), Math.max(lineLeft, lineRight));
            }
            if (fCoerceIn == -1.0f) {
                return Offset.INSTANCE.m4309getUnspecifiedF1C5BW0();
            }
            ...
            return containerLayoutCoordinates.mo5966localPositionOfR5De75A(layoutCoordinates, OffsetKt.Offset(fCoerceIn, centerYForOffset));
        }
        return Offset.INSTANCE.m4309getUnspecifiedF1C5BW0();
    }
    ...
    public static final Rect visibleBounds(LayoutCoordinates layoutCoordinates) {
        ...
        return RectKt.m4332Rect0a9Yr6o(layoutCoordinates.mo5974windowToLocalMKHz9U(rectBoundsInWindow.m4329getTopLeftF1C5BW0()), layoutCoordinates.mo5974windowToLocalMKHz9U(rectBoundsInWindow.m4323getBottomRightF1C5BW0()));
    }
    ...
    public static final boolean m1684containsInclusiveUv8p0NA(Rect rect, long j) {
        float left = rect.getLeft();
        float right = rect.getRight();
        float fM4294getXimpl = Offset.m4294getXimpl(j);
        if (left <= fM4294getXimpl && fM4294getXimpl <= right) {
            float top = rect.getTop();
            float bottom = rect.getBottom();
            float fM4295getYimpl = Offset.m4295getYimpl(j);
            if (top <= fM4295getYimpl && fM4295getYimpl <= bottom) {
                return true;
            }
        }
        return false;
