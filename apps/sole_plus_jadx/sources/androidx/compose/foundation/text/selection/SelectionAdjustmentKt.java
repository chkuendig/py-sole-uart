...
public final class SelectionAdjustmentKt {
    ...
    public static final Selection.AnchorInfo updateSelectionBoundary(final SelectionLayout selectionLayout, final SelectableInfo selectableInfo, Selection.AnchorInfo anchorInfo) {
        ...
        long jM6689getWordBoundaryjx7JFs = selectableInfo.getTextLayoutResult().m6689getWordBoundaryjx7JFs(offset);
        if (!isExpanding(selectableInfo, rawStartHandleOffset, selectionLayout.getIsStartHandle())) {
            return selectableInfo.anchorForOffset(rawStartHandleOffset);
        }
        if (offset == TextRange.m6717getStartimpl(jM6689getWordBoundaryjx7JFs) || offset == TextRange.m6712getEndimpl(jM6689getWordBoundaryjx7JFs)) {
            return updateSelectionBoundary$lambda$1(lazy2);
        }
        return selectableInfo.anchorForOffset(rawStartHandleOffset);
    }
    ...
    public static final Selection.AnchorInfo snapToWordBoundary(SelectableInfo selectableInfo, int i, int i2, int i3, boolean z, boolean z2) {
        int lineStart;
        int lineEnd$default;
        long jM6689getWordBoundaryjx7JFs = selectableInfo.getTextLayoutResult().m6689getWordBoundaryjx7JFs(i2);
        if (selectableInfo.getTextLayoutResult().getLineForOffset(TextRange.m6717getStartimpl(jM6689getWordBoundaryjx7JFs)) == i) {
            lineStart = TextRange.m6717getStartimpl(jM6689getWordBoundaryjx7JFs);
        ...
        if (selectableInfo.getTextLayoutResult().getLineForOffset(TextRange.m6712getEndimpl(jM6689getWordBoundaryjx7JFs)) == i) {
            lineEnd$default = TextRange.m6712getEndimpl(jM6689getWordBoundaryjx7JFs);
        } else if (i >= selectableInfo.getTextLayoutResult().getLineCount()) {
    ...
    private static final Selection.AnchorInfo anchorOnBoundary(SelectableInfo selectableInfo, boolean z, boolean z2, int i, BoundaryFunction boundaryFunction) {
        ...
        long jMo1631getBoundaryfzxv0v0 = boundaryFunction.mo1631getBoundaryfzxv0v0(selectableInfo, rawStartHandleOffset);
        return selectableInfo.anchorForOffset(z ^ z2 ? TextRange.m6717getStartimpl(jMo1631getBoundaryfzxv0v0) : TextRange.m6712getEndimpl(jMo1631getBoundaryfzxv0v0));
    }
