...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6717getStartimpl(j);
        this.selectionEnd = TextRange.m6712getEndimpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        checkRange(TextRange.m6717getStartimpl(j), TextRange.m6712getEndimpl(j));
    }
    ...
    public final void delete(int start, int end) {
        ...
        PartialGapBuffer.replace$default(this.gapBuffer, TextRange.m6715getMinimpl(jTextRange), TextRange.m6714getMaximpl(jTextRange), "", 0, 0, 24, null);
        long jM1448updateRangeAfterDeletepWDy79M = EditingBufferKt.m1448updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m6717getStartimpl(jM1448updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m6712getEndimpl(jM1448updateRangeAfterDeletepWDy79M));
        if (hasComposition()) {
            long jM1448updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m1448updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m6711getCollapsedimpl(jM1448updateRangeAfterDeletepWDy79M2)) {
                commitComposition();
            } else {
                this.compositionStart = TextRange.m6715getMinimpl(jM1448updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m6714getMaximpl(jM1448updateRangeAfterDeletepWDy79M2);
            }
        }
        this.highlight = null;
    }
    ...
    public final void m1447setHighlightK7f2yys(int type, int start, int end) {
        if (start >= end) {
            throw new IllegalArgumentException("Do not set reversed or empty range: " + start + " > " + end);
        }
        this.highlight = new Pair<>(TextHighlightType.m1417boximpl(type), TextRange.m6705boximpl(TextRangeKt.TextRange(RangesKt.coerceIn(start, 0, getLength()), RangesKt.coerceIn(end, 0, getLength()))));
    }
