...
public final class ContentInViewNode extends Modifier.Node implements BringIntoViewResponder, LayoutAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    private long viewportSize = IntSize.INSTANCE.m7431getZeroYbymL2g();
    ...
    public Rect calculateRectForParent(Rect localRect) {
        if (IntSize.m7424equalsimpl0(this.viewportSize, IntSize.INSTANCE.m7431getZeroYbymL2g())) {
            throw new IllegalStateException("Expected BringIntoViewRequester to not be used before parents are placed.".toString());
        }
        return m701computeDestinationO0kMr_c(localRect, this.viewportSize);
    }
    ...
    public Object bringChildIntoView(Function0<Rect> function0, Continuation<? super Unit> continuation) {
        ...
        if (rectInvoke == null || m703isMaxVisibleO0kMr_c$default(this, rectInvoke, 0L, 1, null)) {
    ...
    public void mo667onRemeasuredozmzZPI(long size) {
        ...
        if (m699compareToTemP2vQ(size, j) < 0 && (focusedChildBounds = getFocusedChildBounds()) != null) {
            Rect rect = this.focusedChildBoundsFromPreviousRemeasure;
            if (rect == null) {
                rect = focusedChildBounds;
            }
            if (!this.isAnimationRunning && !this.trackingFocusedChild && m702isMaxVisibleO0kMr_c(rect, j) && !m702isMaxVisibleO0kMr_c(focusedChildBounds, size)) {
                ...
                if (i == 0) {
                    ...
                    Function1<Float, Unit> function1 = new Function1<Float, Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.1
                        ...
                        public final void invoke(float f) {
                            ...
                            float fM802toFloatk4lQ0M = f2 * scrollingLogic.m802toFloatk4lQ0M(scrollingLogic.m800reverseIfNeededMKHz9U(nestedScrollScope.mo758scrollByOzD1aCk(scrollingLogic.m800reverseIfNeededMKHz9U(scrollingLogic.m803toOffsettuRUvjQ(f2 * f)), NestedScrollSource.INSTANCE.m5721getUserInputWNlRxjI())));
                            if (Math.abs(fM802toFloatk4lQ0M) < Math.abs(f)) {
                                JobKt__JobKt.cancel$default(job, "Scroll animation cancelled because scroll was not consumed (" + fM802toFloatk4lQ0M + " < " + f + ')', null, 2, null);
                            }
                        }
                    };
                    ...
                    if (updatableAnimationState.animateToZero(function1, new Function0<Unit>() { // from class: androidx.compose.foundation.gestures.ContentInViewNode.launchAnimation.2.1.2
                        ...
                        public final void invoke2() {
                            ...
                            while (true) {
                                ...
                                if (!(rectInvoke == null ? true : ContentInViewNode.m703isMaxVisibleO0kMr_c$default(contentInViewNode3, rectInvoke, 0L, 1, null))) {
                                    break;
                                }
                                ...
                                continuation.resumeWith(Result.m9087constructorimpl(unit));
                            }
                            if (contentInViewNode2.trackingFocusedChild) {
                                Rect focusedChildBounds = contentInViewNode2.getFocusedChildBounds();
                                if (focusedChildBounds != null && ContentInViewNode.m703isMaxVisibleO0kMr_c$default(contentInViewNode2, focusedChildBounds, 0L, 1, null)) {
    ...
    public final float calculateScrollDelta(BringIntoViewSpec bringIntoViewSpec) {
        ...
        long jM7438toSizeozmzZPI = IntSizeKt.m7438toSizeozmzZPI(this.viewportSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getTop(), rectFindBringIntoViewRequest.getBottom() - rectFindBringIntoViewRequest.getTop(), Size.m4360getHeightimpl(jM7438toSizeozmzZPI));
        }
        if (i == 2) {
            return bringIntoViewSpec.calculateScrollDistance(rectFindBringIntoViewRequest.getLeft(), rectFindBringIntoViewRequest.getRight() - rectFindBringIntoViewRequest.getLeft(), Size.m4363getWidthimpl(jM7438toSizeozmzZPI));
        }
    ...
    private final Rect findBringIntoViewRequest() {
        ...
        if (size > 0) {
            ...
            do {
                ...
                if (rectInvoke != null) {
                    if (m700compareToiLBOSCw(rectInvoke.m4327getSizeNHjbRc(), IntSizeKt.m7438toSizeozmzZPI(this.viewportSize)) > 0) {
    ...
    private final Rect m701computeDestinationO0kMr_c(Rect childBounds, long containerSize) {
        return childBounds.m4331translatek4lQ0M(Offset.m4303unaryMinusF1C5BW0(m704relocationOffsetBMxPBkI(childBounds, containerSize)));
    }

    /* renamed from: isMaxVisible-O0kMr_c$default, reason: not valid java name */
    static /* synthetic */ boolean m703isMaxVisibleO0kMr_c$default(ContentInViewNode contentInViewNode, Rect rect, long j, int i, Object obj) {
        if ((i & 1) != 0) {
            j = contentInViewNode.viewportSize;
        }
        return contentInViewNode.m702isMaxVisibleO0kMr_c(rect, j);
    }

    /* renamed from: isMaxVisible-O0kMr_c, reason: not valid java name */
    private final boolean m702isMaxVisibleO0kMr_c(Rect rect, long j) {
        long jM704relocationOffsetBMxPBkI = m704relocationOffsetBMxPBkI(rect, j);
        return Math.abs(Offset.m4294getXimpl(jM704relocationOffsetBMxPBkI)) <= 0.5f && Math.abs(Offset.m4295getYimpl(jM704relocationOffsetBMxPBkI)) <= 0.5f;
    }

    /* renamed from: relocationOffset-BMxPBkI, reason: not valid java name */
    private final long m704relocationOffsetBMxPBkI(Rect childBounds, long containerSize) {
        long jM7438toSizeozmzZPI = IntSizeKt.m7438toSizeozmzZPI(containerSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return OffsetKt.Offset(0.0f, requireBringIntoViewSpec().calculateScrollDistance(childBounds.getTop(), childBounds.getBottom() - childBounds.getTop(), Size.m4360getHeightimpl(jM7438toSizeozmzZPI)));
        }
        if (i == 2) {
            return OffsetKt.Offset(requireBringIntoViewSpec().calculateScrollDistance(childBounds.getLeft(), childBounds.getRight() - childBounds.getLeft(), Size.m4363getWidthimpl(jM7438toSizeozmzZPI)), 0.0f);
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-TemP2vQ, reason: not valid java name */
    private final int m699compareToTemP2vQ(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Intrinsics.compare(IntSize.m7425getHeightimpl(j), IntSize.m7425getHeightimpl(j2));
        }
        if (i == 2) {
            return Intrinsics.compare(IntSize.m7426getWidthimpl(j), IntSize.m7426getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-iLBOSCw, reason: not valid java name */
    private final int m700compareToiLBOSCw(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Float.compare(Size.m4360getHeightimpl(j), Size.m4360getHeightimpl(j2));
        }
        if (i == 2) {
            return Float.compare(Size.m4363getWidthimpl(j), Size.m4363getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }
