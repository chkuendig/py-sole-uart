...
public final class PagerSnapLayoutInfoProviderKt {
    ...
    public static final float dragGestureDelta(PagerState pagerState) {
        if (pagerState.getLayoutInfo().getOrientation() == Orientation.Horizontal) {
            return Offset.m4294getXimpl(pagerState.m1233getUpDownDifferenceF1C5BW0$foundation_release());
        }
        return Offset.m4295getYimpl(pagerState.m1233getUpDownDifferenceF1C5BW0$foundation_release());
    }
    ...
    public static final float calculateFinalSnappingBound(PagerState pagerState, LayoutDirection layoutDirection, float f, float f2, float f3, float f4) {
        ...
        if (FinalSnappingItem.m824equalsimpl0(iCalculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m828getClosestItembbeMdSM())) {
            if (Math.abs(f5) > f) {
        ...
        } else if (!FinalSnappingItem.m824equalsimpl0(iCalculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m829getNextItembbeMdSM())) {
            if (FinalSnappingItem.m824equalsimpl0(iCalculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m830getPreviousItembbeMdSM())) {
                return f3;
            }
            return 0.0f;
        }
