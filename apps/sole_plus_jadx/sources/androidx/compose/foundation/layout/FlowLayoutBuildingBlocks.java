...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        wrapEllipsisInfoEllipsisInfo$foundation_layout_release.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m332getFirstimpl(wrapEllipsisInfoEllipsisInfo$foundation_layout_release.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
    ...
    public final WrapInfo m908getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        ...
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m332getFirstimpl(leftOver) - IntIntPair.m332getFirstimpl(nextSize.getPackedValue()) < 0)) {
            ...
            return new WrapInfo(true, m908getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m328constructorimpl(Constraints.m7208getMaxWidthimpl(this.constraints), (IntIntPair.m333getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m325boximpl(IntIntPair.m328constructorimpl(IntIntPair.m332getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m333getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int iMax = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m333getSecondimpl(nextSize.getPackedValue()));
        IntIntPair intIntPairM913ellipsisSizeF35zmw$foundation_layout_release = isEllipsisWrap ? null : this.overflow.m913ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, iMax);
        if (intIntPairM913ellipsisSizeF35zmw$foundation_layout_release != null) {
            intIntPairM913ellipsisSizeF35zmw$foundation_layout_release.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m332getFirstimpl(leftOver) - IntIntPair.m332getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m332getFirstimpl(intIntPairM913ellipsisSizeF35zmw$foundation_layout_release.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo wrapInfoM908getWrapInfoOpUlnko = m908getWrapInfoOpUlnko(false, 0, IntIntPair.m328constructorimpl(Constraints.m7208getMaxWidthimpl(this.constraints), (IntIntPair.m333getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m333getSecondimpl(nextSize.getPackedValue()))), intIntPairM913ellipsisSizeF35zmw$foundation_layout_release, lineIndex + 1, iMax, 0, true, true);
                return new WrapInfo(wrapInfoM908getWrapInfoOpUlnko.getIsLastItemInContainer(), wrapInfoM908getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
