...
final /* data */ class BoxMeasurePolicy implements MeasurePolicy {
    ...
    public MeasureResult mo342measure3p2s80s(final MeasureScope measureScope, final List<? extends Measurable> list, long j) {
        ...
        if (list.isEmpty()) {
            return MeasureScope.layout$default(measureScope, Constraints.m7210getMinWidthimpl(j), Constraints.m7209getMinHeightimpl(j), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.layout.BoxMeasurePolicy$measure$1
        ...
        long jM7198copyZbe2FdA$default = this.propagateMinConstraints ? j : Constraints.m7198copyZbe2FdA$default(j, 0, 0, 0, 0, 10, null);
        if (list.size() == 1) {
            final Measurable measurable = list.get(0);
            if (!BoxKt.getMatchesParentSize(measurable)) {
                placeableMo5957measureBRTryo0 = measurable.mo5957measureBRTryo0(jM7198copyZbe2FdA$default);
                iM7210getMinWidthimpl = Math.max(Constraints.m7210getMinWidthimpl(j), placeableMo5957measureBRTryo0.getWidth());
                iM7209getMinHeightimpl = Math.max(Constraints.m7209getMinHeightimpl(j), placeableMo5957measureBRTryo0.getHeight());
            } else {
                iM7210getMinWidthimpl = Constraints.m7210getMinWidthimpl(j);
                iM7209getMinHeightimpl = Constraints.m7209getMinHeightimpl(j);
                placeableMo5957measureBRTryo0 = measurable.mo5957measureBRTryo0(Constraints.INSTANCE.m7218fixedJhjzzOo(Constraints.m7210getMinWidthimpl(j), Constraints.m7209getMinHeightimpl(j)));
            }
            final int i = iM7210getMinWidthimpl;
            final int i2 = iM7209getMinHeightimpl;
        ...
        intRef.element = Constraints.m7210getMinWidthimpl(j);
        final Ref.IntRef intRef2 = new Ref.IntRef();
        intRef2.element = Constraints.m7209getMinHeightimpl(j);
        int size = list.size();
        boolean z = false;
        for (int i3 = 0; i3 < size; i3++) {
            ...
            if (BoxKt.getMatchesParentSize(measurable2)) {
                ...
                placeableArr[i3] = placeableMo5957measureBRTryo02;
                intRef.element = Math.max(intRef.element, placeableMo5957measureBRTryo02.getWidth());
                intRef2.element = Math.max(intRef2.element, placeableMo5957measureBRTryo02.getHeight());
            }
        }
        if (z) {
            ...
            for (int i4 = 0; i4 < size2; i4++) {
                ...
                if (BoxKt.getMatchesParentSize(measurable3)) {
                    placeableArr[i4] = measurable3.mo5957measureBRTryo0(jConstraints);
                }
            }
        }
