...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo395measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        int iM7210getMinWidthimpl = iRound != Integer.MAX_VALUE ? iRound : Constraints.m7210getMinWidthimpl(j);
        int iM7209getMinHeightimpl = iRound2 != Integer.MAX_VALUE ? iRound2 : Constraints.m7209getMinHeightimpl(j);
        if (iRound == Integer.MAX_VALUE) {
            iRound = Constraints.m7208getMaxWidthimpl(j);
        }
        if (iRound2 == Integer.MAX_VALUE) {
            iRound2 = Constraints.m7207getMaxHeightimpl(j);
        }
        final Placeable placeableMo5957measureBRTryo0 = measurable.mo5957measureBRTryo0(ConstraintsKt.Constraints(iM7210getMinWidthimpl, iRound, iM7209getMinHeightimpl, iRound2));
        return MeasureScope.layout$default(measureScope, placeableMo5957measureBRTryo0.getWidth(), placeableMo5957measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.lazy.ParentSizeNode$measure$1
            {
                super(1);
            }
