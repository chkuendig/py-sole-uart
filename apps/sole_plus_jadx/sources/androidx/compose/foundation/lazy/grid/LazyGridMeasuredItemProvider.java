...
public abstract class LazyGridMeasuredItemProvider implements LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo1124createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);
    ...
    public LazyGridMeasuredItem mo1099getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m1129getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }

    /* renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m1129getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        ...
        List<Placeable> listMo1156measure0kLqBqw = this.measureScope.mo1156measure0kLqBqw(index, constraints);
        if (Constraints.m7206getHasFixedWidthimpl(constraints)) {
            iM7209getMinHeightimpl = Constraints.m7210getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m7205getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height".toString());
            }
            iM7209getMinHeightimpl = Constraints.m7209getMinHeightimpl(constraints);
        }
        return mo1124createItemO3s9Psw(index, key, contentType, iM7209getMinHeightimpl, mainAxisSpacing, listMo1156measure0kLqBqw, constraints, lane, span);
    }
