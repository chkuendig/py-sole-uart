...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM7383getXimpl = IntOffset.m7384getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            iM7383getXimpl = IntOffset.m7383getXimpl(lazyGridItemInfo2.getOffset());
        }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (z) {
                        iM7426getWidthimpl = IntSize.m7425getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        iM7426getWidthimpl = IntSize.m7426getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    iMax = Math.max(iMax, iM7426getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
