...
final class DefaultPagerNestedScrollConnection implements NestedScrollConnection {
    ...
    public final long m1217consumeOnOrientationQWom1Mo(long j, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return Velocity.m7489copyOhffZ5M$default(j, 0.0f, 0.0f, 2, null);
        }
        return Velocity.m7489copyOhffZ5M$default(j, 0.0f, 0.0f, 1, null);
    }

    @Override // androidx.compose.ui.input.nestedscroll.NestedScrollConnection
    /* renamed from: onPreScroll-OzD1aCk */
    public long mo1061onPreScrollOzD1aCk(long available, int source) {
        if (NestedScrollSource.m5709equalsimpl0(source, NestedScrollSource.INSTANCE.m5721getUserInputWNlRxjI()) && Math.abs(this.state.getCurrentPageOffsetFraction()) > 1.0E-6d) {
            ...
            float fM4295getYimpl = -this.state.dispatchRawDelta(-RangesKt.coerceIn(this.orientation == Orientation.Horizontal ? Offset.m4294getXimpl(available) : Offset.m4295getYimpl(available), currentPageOffsetFraction, pageSize));
            float fM4294getXimpl = this.orientation == Orientation.Horizontal ? fM4295getYimpl : Offset.m4294getXimpl(available);
            if (this.orientation != Orientation.Vertical) {
                fM4295getYimpl = Offset.m4295getYimpl(available);
            }
            return Offset.m4287copydBAh8RU(available, fM4294getXimpl, fM4295getYimpl);
        }
    ...
    public long mo787onPostScrollDzOQY0M(long consumed, long available, int source) {
        if (NestedScrollSource.m5709equalsimpl0(source, NestedScrollSource.INSTANCE.m5720getSideEffectWNlRxjI()) && m1216mainAxisk4lQ0M(available) != 0.0f) {
    ...
    public Object mo786onPostFlingRZ2iAVY(long j, long j2, Continuation<? super Velocity> continuation) {
        return Velocity.m7484boximpl(m1217consumeOnOrientationQWom1Mo(j2, this.orientation));
    }
