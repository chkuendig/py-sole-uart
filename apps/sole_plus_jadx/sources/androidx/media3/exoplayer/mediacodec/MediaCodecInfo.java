...
public final class MediaCodecInfo {
    ...
    public boolean isVideoSizeAndRateSupportedV21(int i, int i2, double d) {
        ...
        if (Util.SDK_INT >= 29) {
            int iAreResolutionAndFrameRateCovered = Api29.areResolutionAndFrameRateCovered(videoCapabilities, this.mimeType, i, i2, d);
            if (iAreResolutionAndFrameRateCovered == 2) {
                return true;
            }
            if (iAreResolutionAndFrameRateCovered == 1) {
                logNoSupport("sizeAndRate.cover, " + i + "x" + i2 + SdkConstants.PREFIX_RESOURCE_REF + d);
                return false;
            }
        }
        if (!areSizeAndRateSupportedV21(videoCapabilities, i, i2, d)) {
            if (i >= i2 || !needsRotatedVerticalResolutionWorkaround(this.name) || !areSizeAndRateSupportedV21(videoCapabilities, i2, i, d)) {
                logNoSupport("sizeAndRate.support, " + i + "x" + i2 + SdkConstants.PREFIX_RESOURCE_REF + d);
                return false;
            }
            logAssumedSupport("sizeAndRate.rotated, " + i + "x" + i2 + SdkConstants.PREFIX_RESOURCE_REF + d);
        }
        return true;
    }
    ...
    private static final class Api29 {
        ...
        public static int areResolutionAndFrameRateCovered(MediaCodecInfo.VideoCapabilities videoCapabilities, String str, int i, int i2, double d) {
            ...
            if (iEvaluatePerformancePointCoverage == 1 && str.equals("video/avc") && evaluatePerformancePointCoverage(supportedPerformancePoints, new MediaCodecInfo.VideoCapabilities.PerformancePoint(1280, WinError.ERROR_IMAGE_MACHINE_TYPE_MISMATCH_EXE, 60)) != 2) {
                return 0;
            }
