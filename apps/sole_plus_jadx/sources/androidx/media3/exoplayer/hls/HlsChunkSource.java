...
class HlsChunkSource {
    ...
    public void getNextChunk(LoadingInfo loadingInfo, long j, List<HlsMediaChunk> list, boolean z, HlsChunkHolder hlsChunkHolder) {
        ...
        long j5 = jMax;
        this.trackSelection.updateSelectedTrack(j3, j5, j4, list, createMediaChunkIterators(hlsMediaChunk2, j));
        ...
        long initialStartTimeUs = playlistSnapshot.startTimeUs - this.playlistTracker.getInitialStartTimeUs();
        int i2 = iIndexOf;
        Pair<Long, Integer> nextMediaSequenceAndPartIndex = getNextMediaSequenceAndPartIndex(hlsMediaChunk2, z2, playlistSnapshot, initialStartTimeUs, j);
        ...
        if (jLongValue >= playlistSnapshot.mediaSequence || hlsMediaChunk2 == null || !z2) {
        ...
        if (this.cmcdConfiguration != null) {
            ...
            CmcdData.Factory factory2 = new CmcdData.Factory(this.cmcdConfiguration, this.trackSelection, Math.max(0L, j5), loadingInfo.playbackSpeed, "h", !hlsMediaPlaylist.hasEndTag, loadingInfo.rebufferedSince(this.lastChunkRequestRealtimeMs), list.isEmpty());
            if (getIsMuxedAudioAndVideo()) {
                objectType = "av";
            } else {
                objectType = CmcdData.Factory.getObjectType(this.trackSelection);
            }
            CmcdData.Factory objectType2 = factory2.setObjectType(objectType);
            if (iIntValue == -1) {
                jLongValue = jLongValue == -1 ? -1L : jLongValue + 1;
            }
            SegmentBaseHolder nextSegmentHolder2 = getNextSegmentHolder(hlsMediaPlaylist, jLongValue, iIntValue == -1 ? -1 : iIntValue + 1);
            if (nextSegmentHolder2 != null) {
                objectType2.setNextObjectRequest(UriUtil.getRelativePath(UriUtil.resolveToUri(hlsMediaPlaylist.baseUri, segmentBaseHolder.segmentBase.url), UriUtil.resolveToUri(hlsMediaPlaylist.baseUri, nextSegmentHolder2.segmentBase.url)));
                String str = nextSegmentHolder2.segmentBase.byteRangeOffset + SdkConstants.RES_QUALIFIER_SEP;
                ...
                objectType2.setNextRangeRequest(str);
            }
            factory = objectType2;
        } else {
            hlsMediaChunk = hlsMediaChunk2;
        ...
        boolean zShouldSpliceIn = HlsMediaChunk.shouldSpliceIn(hlsMediaChunk, uri, hlsMediaPlaylist, segmentBaseHolder, j2);
        if (zShouldSpliceIn && segmentBaseHolder.isPreload) {
            return;
        }
        hlsChunkHolder.chunk = HlsMediaChunk.createInstance(this.extractorFactory, this.mediaDataSource, this.playlistFormats[i], j2, hlsMediaPlaylist, segmentBaseHolder, uri, this.muxedCaptionFormats, this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), this.isPrimaryTimestampSource, this.timestampAdjusterProvider, this.timestampAdjusterInitializationTimeoutMs, hlsMediaChunk, this.keyCache.get(fullEncryptionKeyUri2), this.keyCache.get(fullEncryptionKeyUri), zShouldSpliceIn, this.playerId, factory);
    }
