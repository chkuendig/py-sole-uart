...
public class MediaCodecVideoRenderer extends MediaCodecRenderer implements VideoSink.RenderControl {
    ...
    private static final int[] STANDARD_LONG_EDGE_VIDEO_PX = {WinError.ERROR_CANT_ACCESS_FILE, 1600, WinError.ERROR_SCREEN_ALREADY_LOCKED, 1280, 960, 854, WinError.ERROR_MULTIPLE_FAULT_VIOLATION, WinError.ERROR_WX86_ERROR, 480};
    ...
    public long getFrameRenderTimeNs(long j, long j2, long j3, float f) {
        long jCalculateEarlyTimeUs = calculateEarlyTimeUs(j2, j3, j, getState() == 2, f, getClock());
        if (isBufferLate(jCalculateEarlyTimeUs)) {
            return -2L;
        }
        if (shouldForceRender(j2, jCalculateEarlyTimeUs)) {
            return -1L;
        }
        if (getState() != 2 || j2 == this.initialPositionUs || jCalculateEarlyTimeUs > MAX_EARLY_US_THRESHOLD) {
            return -3L;
        }
        return this.frameReleaseHelper.adjustReleaseTime(getClock().nanoTime() + (jCalculateEarlyTimeUs * 1000));
    }
    ...
    protected void onStarted() {
        ...
        this.lastRenderRealtimeUs = Util.msToUs(jElapsedRealtime);
    ...
    */
    ...
    protected boolean processOutputBuffer(long j, long j2, MediaCodecAdapter mediaCodecAdapter, ByteBuffer byteBuffer, int i, int i2, int i3, long j3, boolean z, boolean z2, Format format) throws ExoPlaybackException, InterruptedException {
        ...
        long jCalculateEarlyTimeUs = calculateEarlyTimeUs(j, j2, j3, z3, getPlaybackSpeed(), getClock());
        ...
        if (z3 && j != this.initialPositionUs) {
            long jNanoTime2 = getClock().nanoTime();
            long jAdjustReleaseTime = this.frameReleaseHelper.adjustReleaseTime((jCalculateEarlyTimeUs * 1000) + jNanoTime2);
            ...
            if (Util.SDK_INT >= 21) {
                if (j4 < MAX_EARLY_US_THRESHOLD) {
                    if (shouldSkipBuffersWithIdenticalReleaseTime() && jAdjustReleaseTime == this.lastFrameReleaseTimeNs) {
                        skipOutputBuffer(mediaCodecAdapter, i, outputStreamOffsetUs);
                    } else {
    ...
    static final class ReflectiveDefaultVideoFrameProcessorFactory implements VideoFrameProcessor.Factory {
        private static final Supplier<VideoFrameProcessor.Factory> VIDEO_FRAME_PROCESSOR_FACTORY_SUPPLIER = Suppliers.memoize(new Supplier() { // from class: androidx.media3.exoplayer.video.MediaCodecVideoRenderer$ReflectiveDefaultVideoFrameProcessorFactory$$ExternalSyntheticLambda0
            ...
            public final Object get() {
                return MediaCodecVideoRenderer.ReflectiveDefaultVideoFrameProcessorFactory.lambda$static$0();
            }
        });
        ...
        public VideoFrameProcessor create(Context context, DebugViewProvider debugViewProvider, ColorInfo colorInfo, ColorInfo colorInfo2, boolean z, Executor executor, VideoFrameProcessor.Listener listener) throws VideoFrameProcessingException {
            return VIDEO_FRAME_PROCESSOR_FACTORY_SUPPLIER.get().create(context, debugViewProvider, colorInfo, colorInfo2, z, executor, listener);
        }
    }
