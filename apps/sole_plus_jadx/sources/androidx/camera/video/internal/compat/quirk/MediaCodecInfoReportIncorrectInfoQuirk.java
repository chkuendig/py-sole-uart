...
public class MediaCodecInfoReportIncorrectInfoQuirk implements Quirk {
    ...
    public boolean isUnSupportMediaCodecInfo(MediaFormat mediaFormat) {
        ...
        if (isHuaweiMate9() || isRedmiNote8Pro()) {
            return mediaFormatResolver.isVideo() && mediaFormatResolver.isSize(WinBase.LMEM_DISCARDABLE, LMErr.NERR_ProcNoRespond);
        }
        if (isFHDProblematicDevice()) {
            return mediaFormatResolver.isAvc() && mediaFormatResolver.isSize(WinError.ERROR_CANT_ACCESS_FILE, WinError.ERROR_CANNOT_DETECT_DRIVER_FAILURE);
        }
        return false;
    }
    ...
    public static Set<Size> getExtraSupportedSizes() {
        if (isFHDProblematicDevice()) {
            return Collections.singleton(new Size(WinError.ERROR_CANT_ACCESS_FILE, WinError.ERROR_CANNOT_DETECT_DRIVER_FAILURE));
        }
