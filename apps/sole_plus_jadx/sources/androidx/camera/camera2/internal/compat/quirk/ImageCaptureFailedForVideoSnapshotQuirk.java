...
public class ImageCaptureFailedForVideoSnapshotQuirk implements Quirk {
    ...
    private static boolean isUniSocChipsetDevice() {
        return PROBLEMATIC_UNI_SOC_MODELS.contains(Build.MODEL.toLowerCase(Locale.US)) || (Build.VERSION.SDK_INT >= 31 && "Spreadtrum".equalsIgnoreCase(Build.SOC_MANUFACTURER)) || Build.HARDWARE.toLowerCase(Locale.US).startsWith("ums") || ("itel".equalsIgnoreCase(Build.BRAND) && Build.HARDWARE.toLowerCase(Locale.US).startsWith(SdkConstants.UNIT_SP));
    }
