...
class TouchResponse {
    ...
    */
    void processTouchRotateEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) throws Resources.NotFoundException {
        float left;
        float f;
        int top;
        int bottom;
    ...
    void processTouchEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) throws Resources.NotFoundException {
        float fAbs;
        int i;
        float f;
        ...
        if (action == 1) {
            ...
            float f4 = fArr2[1];
            if (f2 != 0.0f) {
                fAbs = xVelocity / fArr2[0];
            } else {
                fAbs = yVelocity / fArr2[1];
            }
    ...
    float getProgressDirection(float dx, float dy) throws Resources.NotFoundException {
    ...
    void scrollUp(float dx, float dy) throws Resources.NotFoundException {
        float f;
        this.mDragStarted = false;
        float progress = this.mMotionLayout.getProgress();
    ...
    void scrollMove(float dx, float dy) throws Resources.NotFoundException {
        float f;
        float progress = this.mMotionLayout.getProgress();
        if (!this.mDragStarted) {
            this.mDragStarted = true;
