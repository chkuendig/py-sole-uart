...
class CamColor {
    ...
    static CamColor fromColorInViewingConditions(int i, ViewingConditions viewingConditions) {
        ...
        float f14 = (1.7f * fPow4) / ((0.007f * fPow4) + DE_MAX);
        float fLog = ((float) Math.log((0.0228f * flRoot2) + DE_MAX)) * 43.85965f;
        double d2 = f13;
    ...
    int viewed(ViewingConditions viewingConditions) {
        ...
        float fSignum3 = Math.signum(((f4 - (f2 * 220.0f)) - (f3 * 6300.0f)) / 1403.0f) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((float) Math.max(0.0d, (Math.abs(r6) * 27.13d) / (400.0d - Math.abs(r6))), 2.380952380952381d));
