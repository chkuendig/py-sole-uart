...
public final class VINResultParser extends ResultParser {
    ...
    private static int modelYear(char c) {
        if (c >= 'E' && c <= 'H') {
            return (c - 'E') + R2.drawable.messenger_bubble_large_white;
        }
        if (c >= 'J' && c <= 'N') {
            return (c - 'J') + R2.drawable.messenger_button_blue_bg_selector;
        }
        if (c == 'P') {
            return R2.drawable.nomessage_icon_color;
        }
        if (c >= 'R' && c <= 'T') {
            return (c - 'R') + R2.drawable.nomessage_icon_white;
        }
        if (c >= 'V' && c <= 'Y') {
            return (c - 'V') + R2.drawable.notification_bg_low;
        }
        if (c >= '1' && c <= '9') {
            return (c - '1') + R2.drawable.notification_bg_normal_pressed;
        }
        if (c < 'A' || c > 'D') {
            throw new IllegalArgumentException();
        }
        return (c - 'A') + R2.drawable.outdoor_base_a;
    }
