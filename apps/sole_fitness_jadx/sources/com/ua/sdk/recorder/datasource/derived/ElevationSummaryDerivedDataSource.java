...
public class ElevationSummaryDerivedDataSource extends DerivedDataSource {
    ...
    protected ElevationSummaryDerivedDataSource(DataSourceIdentifier dataSourceIdentifier, List<DataTypeRef> list) {
    ...
    public void deriveDataPoint(RecorderCalculator recorderCalculator, DataSourceIdentifier dataSourceIdentifier, DataTypeRef dataTypeRef, DataPoint dataPoint) {
        if (this.dataTypeRefTriggers.contains(dataTypeRef) && this.recordData) {
            ...
            DataField dataField = BaseDataTypes.FIELD_ELEVATION_MAX;
            DataField dataField2 = BaseDataTypes.FIELD_ELEVATION_MIN;
            DataField dataField3 = BaseDataTypes.FIELD_ELEVATION_AVG;
            DataField dataField4 = BaseDataTypes.FIELD_ELEVATION_GAIN;
            Double valueDouble = dataPoint.getValueDouble(BaseDataTypes.FIELD_ELEVATION);
            ...
            Double d = this.previousElevation;
            if (d != null && d.doubleValue() < valueDouble.doubleValue()) {
                this.gainMeters += valueDouble.doubleValue() - this.previousElevation.doubleValue();
            }
            this.summaryDataPoint.setValue(dataField4, Double.valueOf(this.gainMeters));
            this.previousElevation = valueDouble;
