...
public class BluetoothCscService implements BaseGattCallback.GattCallbackListener {
    ...
    public void onServicesDiscovered(BluetoothGatt bluetoothGatt, int i) {
        if (i != 0 || bluetoothGatt.getService(UUID.fromString(GattAttributes.CSC_SERVICE)) == null) {
            return;
        }
        BluetoothGattCharacteristic characteristic = bluetoothGatt.getService(UUID.fromString(GattAttributes.CSC_SERVICE)).getCharacteristic(UUID.fromString(GattAttributes.CSC_MEASUREMENT_CHARACTERISTIC));
    ...
    public void onCharacteristicRead(BluetoothGatt bluetoothGatt, BluetoothGattCharacteristic bluetoothGattCharacteristic, int i) {
        ...
        if (i != 0 || !bluetoothGattCharacteristic.getUuid().equals(UUID.fromString(GattAttributes.RSC_MEASUREMENT_CHARACTERISTIC)) || (bluetoothGattCharacteristic.getIntValue(17, 0).intValue() & 1) == 0 || (characteristic = bluetoothGatt.getService(UUID.fromString(GattAttributes.CSC_SERVICE)).getCharacteristic(UUID.fromString(GattAttributes.CONTROL_POINT_CHARACTERISTIC))) == null) {
            return;
        }
    ...
    private void parseCharacteristic(BluetoothGattCharacteristic bluetoothGattCharacteristic) {
        ...
        if (UUID.fromString(GattAttributes.CSC_MEASUREMENT_CHARACTERISTIC).equals(bluetoothGattCharacteristic.getUuid())) {
            ...
            int i = 1;
            if ((iIntValue & 1) != 0) {
                jIntValue = bluetoothGattCharacteristic.getIntValue(20, 1).intValue();
                i = 5;
            } else {
                jIntValue = -1;
            }
            this.listener.onCSCMeasurement(jIntValue, (iIntValue & 2) != 0 ? bluetoothGattCharacteristic.getIntValue(18, i).intValue() : -1L);
        }
    }
}
