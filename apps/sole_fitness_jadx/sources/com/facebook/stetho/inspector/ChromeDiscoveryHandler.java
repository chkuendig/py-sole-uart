...
public class ChromeDiscoveryHandler implements HttpHandler {
    ...
    public boolean handleRequest(SocketLike socketLike, LightHttpRequest lightHttpRequest, LightHttpResponse lightHttpResponse) {
        ...
        try {
            if (PATH_VERSION.equals(path)) {
                ...
                lightHttpResponse.body = LightHttpBody.create("No support for " + path + StringUtils.LF, HTTP.PLAIN_TEXT_TYPE);
            }
            return true;
        } catch (JSONException e) {
            lightHttpResponse.code = 500;
            lightHttpResponse.reasonPhrase = "Internal server error";
            lightHttpResponse.body = LightHttpBody.create(e.toString() + StringUtils.LF, HTTP.PLAIN_TEXT_TYPE);
            return true;
        }
    }
    ...
    private String makeTitle() {
        ...
        if (iIndexOf >= 0) {
            sb.append(processName.substring(iIndexOf));
        }
        return sb.toString();
    }
