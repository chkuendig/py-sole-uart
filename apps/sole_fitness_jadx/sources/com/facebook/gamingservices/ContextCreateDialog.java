...
public class ContextCreateDialog extends FacebookDialogBase<ContextCreateContent, Result> {
    ...
    @Override // com.facebook.internal.FacebookDialogBase
    public void showImpl(ContextCreateContent contextCreateContent, Object obj) throws JSONException {
        if (CloudGameLoginHandler.isRunningInCloud()) {
            showForCloud(contextCreateContent, obj);
        }
    }

    private void showForCloud(ContextCreateContent contextCreateContent, Object obj) throws JSONException {
        ...
        try {
            if (contextCreateContent.getSuggestedPlayerIDs() != null) {
                List listAsList = Arrays.asList(contextCreateContent.getSuggestedPlayerIDs().split("\\s*,\\s*"));
                if (!listAsList.isEmpty()) {
                    if (listAsList.size() == 1) {
                        jSONObject.put("id", listAsList.get(0));
                    } else {
                        JSONArray jSONArray = new JSONArray();
                        for (int i = 0; i < listAsList.size(); i++) {
                            jSONArray.put(listAsList.get(i));
                        }
