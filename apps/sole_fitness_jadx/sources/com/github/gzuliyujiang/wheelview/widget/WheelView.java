...
public class WheelView extends View implements Runnable {
    ...
    private void initAttrs(Context context, AttributeSet attributeSet, int i, int i2) {
        if (attributeSet == null) {
            ...
            this.curtainColor = -1;
            this.curtainCorner = 0;
            this.curtainRadius = 0.0f;
        ...
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, R.styleable.WheelView, i, i2);
    ...
    protected void onAttributeSet(Context context, TypedArray typedArray) {
        ...
        this.curtainCorner = 0;
        this.curtainRadius = typedArray.getDimension(R.styleable.WheelView_wheel_curtainRadius, 0.0f);
    ...
    private void notifyDataSetChanged(int i) {
        int iMax = Math.max(Math.min(i, getItemCount() - 1), 0);
    ...
    private void drawAllItemText(Canvas canvas) {
        ...
        while (i4 < this.defaultItemPosition + i3 + this.drawnItemCount) {
            ...
            float f5 = this.curvedEnabled ? this.drawnCenterYCoordinate - fComputeYCoordinateAtAngle : i8;
            String strObtainItemText = obtainItemText(i4);
            if (this.paint.measureText(strObtainItemText) - getMeasuredWidth() > 0.0f && (length = strObtainItemText.length()) > 5) {
                strObtainItemText = strObtainItemText.substring(0, length - 4) + "...";
            }
            drawItemText(canvas, strObtainItemText, f5);
            i4++;
            i5++;
        }
    }
    ...
    private void handleActionMove(MotionEvent motionEvent) {
        ...
        if (Math.abs(this.downPointYCoordinate - motionEvent.getY()) < this.touchSlop && iComputeDistanceToEndPoint > 0) {
            this.isClick = true;
            return;
        }
        this.isClick = false;
    ...
    public final void scrollTo(final int i) {
        ...
        ValueAnimator valueAnimatorOfInt = ValueAnimator.ofInt(i3, ((i2 - i) * this.itemHeight) + i3);
        valueAnimatorOfInt.setDuration(300L);
        valueAnimatorOfInt.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.github.gzuliyujiang.wheelview.widget.WheelView.1
        ...
        valueAnimatorOfInt.addListener(new AnimatorListenerAdapter() { // from class: com.github.gzuliyujiang.wheelview.widget.WheelView.2
