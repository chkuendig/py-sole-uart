        ...
        if (isDrawingValuesAllowed(this.mChart)) {
            ...
            for (int i2 = 0; i2 < this.mChart.getScatterData().getDataSetCount(); i2++) {
                ...
                if (shouldDrawValues(iScatterDataSet)) {
                    ...
                    int i3 = 0;
                    while (i3 < fArrGenerateTransformedValuesScatter.length && this.mViewPortHandler.isInBoundsRight(fArrGenerateTransformedValuesScatter[i3])) {
                        if (this.mViewPortHandler.isInBoundsLeft(fArrGenerateTransformedValuesScatter[i3])) {
                            int i4 = i3 + 1;
                            if (this.mViewPortHandler.isInBoundsY(fArrGenerateTransformedValuesScatter[i4])) {
                                int i5 = i3 / 2;
                                ?? entryForIndex = iScatterDataSet.getEntryForIndex(this.mXBounds.min + i5);
                                if (iScatterDataSet.isDrawValuesEnabled()) {
                                    i = i3;
                                    mPPointF = mPPointF2;
                                    drawValue(canvas, iScatterDataSet.getValueFormatter(), entryForIndex.getY(), entryForIndex, i2, fArrGenerateTransformedValuesScatter[i3], fArrGenerateTransformedValuesScatter[i4] - fConvertDpToPixel, iScatterDataSet.getValueTextColor(i5 + this.mXBounds.min));
                                } else {
                                    i = i3;
                                    mPPointF = mPPointF2;
                                }
                                if (entryForIndex.getIcon() != null && iScatterDataSet.isDrawIconsEnabled()) {
                                    ...
                                    Utils.drawImage(canvas, icon, (int) (fArrGenerateTransformedValuesScatter[i] + mPPointF.x), (int) (fArrGenerateTransformedValuesScatter[i4] + mPPointF.y), icon.getIntrinsicWidth(), icon.getIntrinsicHeight());
                                }
                            } else {
                                i = i3;
                                mPPointF = mPPointF2;
