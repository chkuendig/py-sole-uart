    ...
    */
    public byte[] nextBytes(byte[] array, int fromIndex, int toIndex) {
        ...
        if (fromIndex >= 0 && length >= fromIndex) {
            z = toIndex >= 0 && array.length >= toIndex;
        }
        if (!z) {
            throw new IllegalArgumentException(("fromIndex (" + fromIndex + ") or toIndex (" + toIndex + ") are out of range: 0.." + array.length + ClassUtils.PACKAGE_SEPARATOR_CHAR).toString());
        }
