...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
    ...
    public Period normalizedStandard(PeriodType periodType) {
        PeriodType periodType2 = DateTimeUtils.getPeriodType(periodType);
        Period period = new Period(getMillis() + (getSeconds() * 1000) + (getMinutes() * 60000) + (getHours() * 3600000) + (getDays() * 86400000) + (getWeeks() * 604800000), periodType2, ISOChronology.getInstanceUTC());
        int years = getYears();
        int months = getMonths();
        if (years != 0 || months != 0) {
            long j = (years * 12) + months;
            if (periodType2.isSupported(DurationFieldType.YEARS_TYPE)) {
                period = period.withYears(FieldUtils.safeToInt(j / 12));
                j -= r0 * 12;
            }
            if (periodType2.isSupported(DurationFieldType.MONTHS_TYPE)) {
                int iSafeToInt = FieldUtils.safeToInt(j);
                j -= iSafeToInt;
                period = period.withMonths(iSafeToInt);
            }
            if (j != 0) {
