    ...
    public static char binaryToHexDigit(boolean[] zArr, int i) {
        ...
        if (zArr.length > i2 && zArr[i2]) {
            ...
            if (zArr.length <= i3 || !zArr[i3]) {
                int i4 = i + 1;
                return (zArr.length <= i4 || !zArr[i4]) ? zArr[i] ? '9' : '8' : zArr[i] ? 'b' : 'a';
            }
            int i5 = i + 1;
            return (zArr.length <= i5 || !zArr[i5]) ? zArr[i] ? 'd' : 'c' : zArr[i] ? 'f' : 'e';
        }
        int i6 = i + 2;
        if (zArr.length <= i6 || !zArr[i6]) {
            int i7 = i + 1;
            return (zArr.length <= i7 || !zArr[i7]) ? zArr[i] ? '1' : '0' : zArr[i] ? '3' : '2';
        }
        int i8 = i + 1;
        return (zArr.length <= i8 || !zArr[i8]) ? zArr[i] ? '5' : '4' : zArr[i] ? '7' : '6';
    }
