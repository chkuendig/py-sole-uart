...
public final class OperatorReplay<T> extends ConnectableObservable<T> {
    ...
    static class BoundedReplayBuffer<T> extends AtomicReference<Node> implements ReplayBuffer<T> {
        ...
        public final void next(T t) {
            Object objEnterTransform = enterTransform(this.nl.next(t));
            long j = this.index + 1;
            this.index = j;
        ...
        public final void error(Throwable th) {
            Object objEnterTransform = enterTransform(this.nl.error(th));
            long j = this.index + 1;
            this.index = j;
            addLast(new Node(objEnterTransform, j));
            truncateFinal();
        ...
        public final void complete() {
            Object objEnterTransform = enterTransform(this.nl.completed());
            long j = this.index + 1;
            this.index = j;
            addLast(new Node(objEnterTransform, j));
            truncateFinal();
            ...
            synchronized (innerProducer) {
                ...
                while (!innerProducer.isUnsubscribed()) {
                    ...
                    while (j != 0 && (node = node2.get()) != null) {
                        ...
                        try {
                            if (this.nl.accept(innerProducer.child, objLeaveTransform)) {
                            ...
                            Exceptions.throwIfFatal(th);
                            innerProducer.unsubscribe();
                            if (this.nl.isError(objLeaveTransform) || this.nl.isCompleted(objLeaveTransform)) {
                                return;
                            }
                            innerProducer.child.onError(OnErrorThrowable.addValueAsLastCause(th, this.nl.getValue(objLeaveTransform)));
                            return;
                        }
                    }
                    if (j2 != 0) {
